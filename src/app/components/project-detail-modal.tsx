...
export function ProjectDetailModal({ project, onClose }: ProjectDetailModalProps) {
  ...
  return (
    <AnimatePresence>
      ...
      {/* 🔥 MODAL CONTAINER - ZOOM + SLIDE ANIMATION */}
      <motion.div
        key="modal"
      ...
      >
        <div className="min-h-full flex items-center justify-center p-3 sm:p-4 md:p-6">
          ...
          <motion.div
            initial={{ y: 50 }}
            animate={{ y: 0 }}
            className="relative bg-[var(--bg-primary)] border-2 border-[var(--accent-primary)]/30 rounded-2xl sm:rounded-3xl w-full max-w-5xl shadow-[0_20px_80px_rgba(0,217,255,0.4)] my-4 sm:my-8 overflow-hidden"
          ...
          >
            ...
            <motion.button
              onClick={handleClose}
              className="fixed top-4 right-4 sm:absolute sm:top-6 sm:right-6 p-2.5 sm:p-3 rounded-xl bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)] hover:border-[var(--accent-primary)]/50 hover:bg-red-500/20 transition-all z-[100002] group shadow-lg"
              whileHover={{ scale: 1.1, rotate: 90 }}
              whileTap={{ scale: 0.9 }}
            >
              <X className="w-5 h-5 sm:w-6 sm:h-6 text-white group-hover:text-red-400 transition-colors" />
            </motion.button>
